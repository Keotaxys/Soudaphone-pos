...
export interface Product {
  ...
  // 🟢 4. ExpenseRecord Interface (ປະຫວັດລາຍຈ່າຍ)
  ...
  // 🟢 5. CustomerOrder Interface (ໃໝ່! - ຕິດຕາມຄຳສັ່ງຊື້)
  export interface CustomerOrder {
    id?: string;
    customerName: string;   // ຊື່ລູກຄ້າ
    date: string;           // ວັນທີສັ່ງ (ISO String)
    productName: string;    // ຊື່ສິນຄ້າ
    source: string;         // ແຫຼ່ງສັ່ງ (ຈີນ/ໄທ/ຫວຽດ/ອື່ນໆ)
    quantity: number;       // ຈຳນວນ
    costPrice: number;      // ຕົ້ນທຶນ
    salePrice: number;      // ລາຄາຂາຍ
    link?: string;          // ລິ້ງສິນຄ້າ
    imageUrl?: string;      // ຮູບສິນຄ້າ
    status: 'ຮັບອໍເດີ້' | 'ສັ່ງເຄື່ອງແລ້ວ' | 'ເຄື່ອງຮອດແລ້ວ' | 'ຈັດສົ່ງສຳເລັດ';
    createdAt: string;
  }
  
  // 🟢 6. COLORS (Theme ສີ Teal + Orange)
  export const COLORS = {
  ...
  // 🟢 7. Helper Functions (ຟັງຊັນຊ່ວຍເຫຼືອ)
