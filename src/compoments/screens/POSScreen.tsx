...
import { CartItem, Product } from '../../types'; // Import ຈາກໄຟລ໌ກາງ

const { width } = Dimensions.get('window');
const COLUMN_COUNT = 2;
const CARD_WIDTH = (width / COLUMN_COUNT) - 20;

// ຮັບ Props ຈາກ index.tsx
            ...
            renderItem={({ item }) => (
                <TouchableOpacity style={styles.card} onPress={() => addToCart(item)} onLongPress={() => openEditProductModal(item)} activeOpacity={0.8}>
                    <View style={styles.imageContainer}>
                        ...
                        <View style={[styles.currencyBadge, { backgroundColor: item.priceCurrency === 'THB' ? '#FFB74D' : '#4DB6AC' }]}><Text style={styles.currencyText}>{item.priceCurrency || 'LAK'}</Text></View>
                        {item.stock <= 5 && <View style={styles.stockBadge}><Text style={styles.stockText}>{item.stock}</Text></View>}
                    </View>
                    <View style={styles.cardContent}>
                        <Text style={styles.title} numberOfLines={1}>{item.name}</Text>
                        <Text style={[styles.price, { color: item.priceCurrency === 'THB' ? '#F57C00' : '#009688' }]}>{formatNumber(item.price)} {item.priceCurrency === 'THB' ? '฿' : '₭'}</Text>
                    </View>
...
const styles = StyleSheet.create({
    ...
    toolBtn: { flexDirection: 'row', backgroundColor: '#FFB74D', padding: 10, borderRadius: 8, alignItems: 'center', gap: 5 },
    toolText: { color: 'white', fontFamily: 'Lao-Bold', fontSize: 12 },
    card: { width: CARD_WIDTH, backgroundColor: 'white', marginBottom: 15, borderRadius: 16, overflow: 'hidden', elevation: 2, shadowColor: '#4DB6AC', shadowOpacity: 0.1, shadowRadius: 5 },
    ...
    stockBadge: { position: 'absolute', top: 8, right: 8, backgroundColor: '#EF5350', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 10 },
    ...
    addBtn: { position: 'absolute', bottom: 10, right: 10, backgroundColor: '#FFB74D', width: 36, height: 36, borderRadius: 18, justifyContent: 'center', alignItems: 'center', shadowColor: '#000', shadowOpacity: 0.2, shadowRadius: 2, elevation: 3 },
